...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    BOOL bFoundVisible;
    int iOrig;
	DWORD dwTextStyle = DT_LEFT | DT_VCENTER | DT_SINGLELINE;    //Text 스타일 정의
	int nReverseCol = 0;
	int nReverseRow = 0;
	int nCOl;
    ...
    case VK_DELETE: 
		...
		for (int row = Selection.GetMinRow(); row < Selection.GetMaxRow() + 1; row++)
		{
			for (int col = Selection.GetMinCol(); col < Selection.GetMaxCol() + 1; col++)
			{
				//타이틀은 지우지말기
				if (col != 0 && row != 0)
				{
...
BOOL CGridCtrl::Save(LPCTSTR filename)
{
    ...
    {		
		...
		CString str00;
		CString str01;
		CString strMissBuff[1000];
		int nMissCnt = 0;
		CString strShortBuff[1000];
		int nShortCnt = 0;
		...
		for (i = 0; i < nNumRows; i++)
        {
            for (int j = 0; j < nNumColumns; j++)
            {
				if (j == 4)
				{
					str00 = GetItemText(i, j);				
					
					CString *ptrSubString00 = new CString[1000];
					int nIndex = 0;
					for (int m = 0; m < 1000; m++)
					{
						AfxExtractSubString(ptrSubString00[m], str00, nIndex++, _T('\n'));
					}
					for (int k = 0; k < nIndex; k++)
					{
						if (ptrSubString00[k] != _T(""))
						{
							strMissBuff[nMissCnt] = ptrSubString00[k];
							nMissCnt++;
						}

					}
					delete[] ptrSubString00;

					File.WriteString(strMissBuff[0]);
					File.WriteString((j == (nNumColumns - 1)) ? _T("\n") : _T(","));

				}
				else if (j == 5)
				{
					str01 = GetItemText(i, j);

					CString *ptrSubString01 = new CString[1000];
					int nIndex1 = 0;
					for (int m = 0; m < 1000; m++)
					{
						AfxExtractSubString(ptrSubString01[m], str01, nIndex1++, _T('\n'));
					}
					for (int k = 0; k < nIndex1; k++)
					{
						if (ptrSubString01[k] != _T(""))
						{
							strShortBuff[nShortCnt] = ptrSubString01[k];
							nShortCnt++;
						}

					}
					delete[] ptrSubString01;

					File.WriteString(strShortBuff[0]);
					File.WriteString((j == (nNumColumns - 1)) ? _T("\n") : _T(","));
				...
				if (j == 7)
				{
					int nMaxCnt = (nMissCnt > nShortCnt ? nMissCnt : nShortCnt);
					for (int d = 1; d < nMaxCnt; d++)
					{
						File.WriteString(_T(",,,,"));
						File.WriteString(strMissBuff[d]);
						File.WriteString(_T(","));
						File.WriteString(strShortBuff[d]);
						File.WriteString(_T(",,\n"));
					}
